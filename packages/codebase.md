...
> Generated from solution at: `/mnt/c/Users/Marcelo.Trenkenchu/source/repos/matren6/aaia/packages`

> Options: Show All Files: ON | Condensed Python: ON
...
│   ├── mandates.py
│   ├── metacognition.py
│   ├── nix_aware_self_modification.py
│   ├── prompt_manager.py
│   ├── prompt_optimizer.py
│   ├── router.py
│   ├── scheduler.py
...
│   │   ├── capabilty_analysis_new.json
│   │   ├── code_improvement_analysis.json
│   │   ├── code_review.json
│   │   ├── economic_strategy.json
│   │   ├── environment_analysis.json
│   │   ├── evolution_lessons.json
│   │   ├── improvement_opportunity.json
│   │   ├── intent_prediction.json
│   │   ├── metacognition_reflection.json
│   │   ├── process_optimization.json
│   │   ├── prompt_optimization.json
│   │   ├── risk_analysis.json
│   │   ├── strategy_optimization.json
│   │   ├── system_assessment_synthesis.json
│   │   ├── task_execution_planner.json
│   │   └── thinking_patterns.json
│   ├── dialogue/
│   │   ├── command_understanding.json
│   │   └── command_understanding_optimization.json
│   ├── generation/
│   │   ├── code_improvement.json
│   │   ├── code_repair.json
│   │   ├── evolution_plan.json
│   │   ├── evolution_planning.json
│   │   ├── forge_code_generation.json
│   │   ├── goal_planning.json
│   │   └── tool_creation.json
│   ├── goals/
...
### `modules/nix_aware_self_modification.py`
...
class Arbiter:
    def __init__(self):
        ...
        print(json.dumps(asdict(self), indent=2, default=str))
...
class ModelInfo:
    ...
    def total_cost_per_1k(self):
        -> float

    def meets_requirements(self, task_type, min_context, max_cost):
        -> bool
...
class BaseLLMProvider:
    ...
    def select_optimal_model(self, task_type, complexity, max_cost):
        -> str
...
### `llm/ollama_capabilities.py`

```python
def extract_capabilities_from_show(show_response):
    -> Dict[str, bool]

def _is_code_optimized(model_name, family):
    -> bool

def _supports_function_calling(model_name, family):
    -> bool

def extract_context_window(show_response):
    -> int

def _parse_param_size(size_str):
    -> float

def calculate_speed_score(details):
    -> int

def get_model_description(details):
    -> str
```
...
class OllamaAPIClient:
    ...
    def list_models(self, force_refresh):
        -> List[Dict]

    def get_model_details(self, model_name, use_cache):
        -> Dict
...
class VeniceAPIClient:
    ...
    def get_rate_limits(self, force_refresh):
        -> Dict

    def get_models(self, force_refresh):
        -> list

    def get_compatibility_mapping(self):
        -> Dict

    def resolve_model_name(self, model):
        -> str

    def get_model_pricing(self, model_id):
        -> Optional[Dict]
